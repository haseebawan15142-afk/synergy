...
export function MegaMenu({ label, href, menu, active, align = "center" }: MegaMenuProps) {
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            className={cn("absolute top-full z-50 w-[min(52rem,calc(100vw-2rem))] pt-3", panelAlign)}
          >
              ...
              <div className="grid grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
                ...
                >
                  {menu.columns.map((column, colIndex) => (
                    <div key={`${column.heading}-${colIndex}`} className={colIndex > 0 ? "" : ""}>
